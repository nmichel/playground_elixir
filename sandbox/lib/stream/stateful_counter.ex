defmodule Tuto.Stream.StatefulCounter do
  defstruct base: 0, spit_period_ms: 10, pid: nil

  def new(base \\ 0, spit_period_ms \\ 10)  do
    {:ok, pid} = Agent.start fn -> base end
    %__MODULE__{base: base, spit_period_ms: spit_period_ms, pid: pid}
  end

  def get(%__MODULE__{spit_period_ms: spit_period_ms, pid: pid}) do
    :ok = Process.sleep spit_period_ms
    Agent.get pid, fn counter -> counter end
  end

  def get_and_inc(%__MODULE__{spit_period_ms: spit_period_ms, pid: pid}) do
    :ok = Process.sleep spit_period_ms
    Agent.get_and_update pid, fn counter -> {counter, counter+1} end
  end
end

defimpl Enumerable, for: Tuto.Stream.StatefulCounter do
